...
%if %kpatch
%if %sublevel
%define tar_ver   	%{kernelversion}.%{patchlevel}-sabreboard
%else
%define tar_ver		%{kernelversion}.%(expr %{patchlevel} - 1)
%endif
%define patch_ver 	%{kversion}-%{kpatch}-%{ktag}%{mibrel}
%else
%define tar_ver   	%{kernelversion}.%{patchlevel}.%{sublevel}-sabreboard
...
%{?_with_sabreboard: %global build_sabreboard 1}
...
%{?_without_sabreboard: %global build_sabreboard 0}
...
%endif
%if %build_sabreboard
%define summary_sabreboard Linux Kernel for Wandboard arm machines
%define info_sabreboard This kernel is compiled for Wandboard boards. It will run on Wandboard boards.
%mkflavour sabreboard
%endif
%endif
...
BuildKernel() {
	...
	if [ "$KernelVer" = "%{kversion}-sabreboard-%{buildrpmrel}" ]; then
		%kmake dtbs
	fi
	...
	%if %{build_sabreboard}
		...
		install -m644 arch/arm/boot/dts/*sabre*.dtb %{temp_boot}/dtbs
        %endif
...
SaveDevel() {
	...
	sed -i 's!include $(AQROOT)/config!!g' drivers/mxc/gpu-viv/Kbuild
